...
class Run:
	...
	def setName(self, fileName, runNumber):
		self.name = fileName
...
class DataCampaign:
	...
	def setup(self, args, path):
		...
		#for i in range(len(args)):
			...
			#self.fileNames.append(str(args[i+2]))
			#print args[i+2]
			runi = Run()
			#runi.setup()
			##print args[i+2]
			runi.setName("gdc" + str(args[i+2])+ ".000", int(args[i+2]))
...
class DATEReader:
	...
	def readBinary(self, run, dataFormat="cosmic"):
